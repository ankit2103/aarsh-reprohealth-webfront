...
const StoriesTab = ({ doctorInfo, setDoctorInfo }) => {
  const [loading, setLoading] =useState(false);
  const { doctorId } = useParams();
  const [formData, setFormData] = useState({
    rating: 0,
    description: "",
    doctorId: doctorId || doctorInfo?._id,
  });
  const [isShareOpen, setIsShareOpen] = useState(false);
  // const { doctorId, labId, clinicId, rating, description } = req.body;
  console.log("StoriesTab doctorInfo----------------", doctorInfo);
  ...
  const handleReviewSubmit = async (e) => {
    e.preventDefault();
    const { rating, description, doctorId } = formData;
    if (!description || !rating || doctorId <= 0) {
      if (!description) {
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        try {
          ...
          console.error("Error while fetching doctor details:", err);
        }
      }
      setLoading(false);
  ...
  const fetchDetailById = async () => {
    try {
      const result = await fetchDoctorById({ doctorId: doctorId });
      if (result?.code === 200) {
        setLoading(false);
  ...
  return (
    <div className="">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-4">
        {doctorInfo?.reviewsArray.length > 0 &&
          doctorInfo?.reviewsArray.map((item, index) => {
