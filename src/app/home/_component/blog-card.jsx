  ...
  useEffect(() => {
    const getBlogs = async () => {
      try {
        ...
        if (result?.code === 200) {
          const filteredBlogs = result.data?.filter(
            (item) => item?.bcId?.name !== "Our Story"
          );
    ...
    <div className="w-full mx-auto pb-10 relative">
      <div className="main-container">
        <div className="container">
          <div className="flex flex-col md:flex-row md:items-center justify-between w-full px-2 mb-6">
            ...
            <div className="mt-10 animate-pulse">
              <div className="flex overflow-x-scroll gap-6">
                {[...Array(3)].map((_, index) => (
                  <div
                    key={index}
                    className="min-w-[400px] p-3 bg-gray-100 rounded-2xl"
                  >
              ...
              <Slider {...sliderSettings}>
                {Array.isArray(blogs) && blogs?.length>0 && blogs.map((blog) => (
                  <div key={blog._id} className="px-2">
                    <div className="bg-white shadow rounded-2xl min-h-[480px] h-[420px] flex flex-col p-4">
                      ...
                      <p className="mt-2 font-semibold text-md line-clamp-1">
                        {blog.title}
                      </p>
                      <div className="text-[var(--greyP)] mt-2 text-sm line-clamp-3">
