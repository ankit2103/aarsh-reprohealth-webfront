...
export const LabTestHeaders = [
  ...
  { key: "testName", label: "Test", sortable: true, visible: true },
...
const TestResults = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((row) =>
      row.labId?.name?.toLowerCase().includes(searchQuery.toLowerCase())
    );
  }, [data, searchQuery]);
  ...
  //     try {
  //       const result = await getUserAllBookedLabTestRecords();
  //       if (result?.code === 201 && Array.isArray(result?.data)) {
  //         setData(result?.data);
  //         console.log("Data--------------------------", result?.data);
  //       }
  //     } catch (error) {
  //       console.log("useEffect getUserAllBookedLabTestRecords Error ", error);
  //     }
  //   };
  //   fetchData();
  // }, []);
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.error("useEffect getUserAllBookedLabTestRecords Error", error);
      } finally {
