...
import { fetchBlogs, fetchBlogById, fetchStoryById, fetchOurStory } from "../../../../utils/blog/blog.util";
...
const BlogPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const blogData = await fetchStoryById(id);
        ...
        const filtered = result?.data.reverse()?.filter(
          (b) => b._id !== id && b?.bcId?.name === category
        ) || [];
  ...
  return (
    <div className="container mx-auto px-4 py-8 mb-8 mt-8">
      ...
      <Slider {...sliderSettings}>
        {loading ? (
          <div className="h-[260px] md:h-[500px] w-full bg-gray-200 rounded-xl animate-pulse" />
        ) : (
                  ...
                  relatedBlogs.slice(0, 5).map((related) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <Link
                          href={`/our-story/${related.slug}/${related._id}`}
